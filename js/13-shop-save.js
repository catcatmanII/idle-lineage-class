...
function openSlotSelect(mode){
    _slotMode = mode;
    { let _ct = document.getElementById('create-classic-toggle'); if (_ct && mode === 'new') _ct.checked = false; let _tt = document.getElementById('create-traditional-toggle'); if (_tt && mode === 'new') _tt.checked = false; }   // 🎮🏛️ 創角流程重置經典＋傳統開關（預設皆關閉，二者獨立）
...
function startGame() {
    ...
    player.mapModOn = false;   // 🗺️ 地圖詞綴：改由 NPC 紮那手動開關（需 30 等）
...
function loadGame() {
    ...
    if (s) {
        ...
        if (!mapState.modifiers) mapState.modifiers = { prefixes:[], suffixes:[], nextRefreshAt:0, active:false };
        if (!mapState.savedModifiers) mapState.savedModifiers = {};
