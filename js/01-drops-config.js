...
const SAVE_DEFAULTS = {
    ...
    magicShieldCd: 0, reviveScrollCd: 0, lastMapByCat: {}, lastBattleMap: null, tracking: null, ismaelAccUsed: false, sherineWorld: false, sherineMad: false, classicMode: false, traditionalMode: false, mapModOn: false, _mapModAutoEnabled: false, _mapModOptIn: false,
    ...
    statuses:{ stun:0, freeze:0, stone:0, poison:0, poisonDmg:0, poisonTick:0, burn:0, burnDmg:0, burnTick:0,
               scald:0, scaldDmg:0, scaldTick:0, bleed:0, bleedDmg:0, bleedTick:0, sleep:0, silence:0, paralyze:0, magicseal:0, armorBreak:0, slowAtk:0, slowPct:0, cleave:0 },
...
const MASTERY_DATA = {
    ...
    dark: { logo: 'assets/logo/黑暗妖精logo.png', boss: '巴風特', list: {
        d_poison: { n: '劇毒精通', pos: 'top',    msg: '附加劇毒必定觸發、每層200%傷害',     d: '附加劇毒觸發機率變成100%，且每層中毒每秒造成該次攻擊200%傷害（持續5秒，可疊層）' },
...
let _gameSpeedIdx = 0;              // ⚙️ 遊戲速度索引：0=1x, 1=3x, 2=9x
const _gameSpeeds = [1, 3, 9, 16];      // ⚙️ 對應的速度倍率
const _gameSpeedLabels = ['1x', '3x', '9x', '16x'];

function toggleGameSpeed() {        // ⚙️ 點擊齒輪依序 1x → 3x → 9x → 1x
...
function _applyGameSpeed() {
    ...
    if (elIcon) elIcon.style.color = speed >= 9 ? '#facc15' : '';
    if (_gameLoopId !== null) clearInterval(_gameLoopId);
    _gameLoopId = setInterval(gameLoop, Math.round(100 / speed));
}
...
function startGameTimers() {
    ...
    _applyGameSpeed();   // ⚙️ 依目前速度啟動 gameLoop
...
let mapState = { current: "training", mobs: [null, null, null, null, null], targetIdx: 0, modifiers: { prefixes:[], suffixes:[], nextRefreshAt:0, active:false }, savedModifiers: {} };   // 🆕 5 格（前排 0,1,2＋後排 3,4）
...
const ATK_APM = {
    '王子':       { '單手劍':57.6,  '單手鈍器':65.45, '弓':36,    '十字弓':36,    '單手矛':68,   '雙手矛':66,   '魔杖':72, '匕首':60, '雙手劍':65,    '雙手鈍器':65,    '雙刀':60,   '鋼爪':60,   '鎖鏈劍':60, '雙斧':60,    '奇古獸':60 },
    '公主':       { '單手劍':65.45, '單手鈍器':57.6,  '弓':37.89, '十字弓':37.89, '單手矛':57.6, '雙手矛':56,   '魔杖':63, '匕首':68, '雙手劍':60,    '雙手鈍器':60,    '雙刀':60,   '鋼爪':60,   '鎖鏈劍':60, '雙斧':60,    '奇古獸':60 },
    '男騎士':     { '單手劍':65.45, '單手鈍器':65.45, '弓':30,    '十字弓':30,    '單手矛':65,   '雙手矛':63,   '魔杖':72, '匕首':68, '雙手劍':65,    '雙手鈍器':65,    '雙刀':60,   '鋼爪':60,   '鎖鏈劍':60, '雙斧':60,    '奇古獸':60 },
    '女騎士':     { '單手劍':62.61, '單手鈍器':62.61, '弓':31.3,  '十字弓':31.3,  '單手矛':60,   '雙手矛':58,   '魔杖':69, '匕首':65, '雙手劍':62.61, '雙手鈍器':62.61, '雙刀':60,   '鋼爪':60,   '鎖鏈劍':60, '雙斧':60,    '奇古獸':60 },
    '男妖精':     { '單手劍':72,    '單手鈍器':55.38, '弓':60,    '十字弓':60,    '單手矛':55.4, '雙手矛':54,   '魔杖':69, '匕首':75, '雙手劍':55,    '雙手鈍器':55,    '雙刀':60,   '鋼爪':60,   '鎖鏈劍':55, '雙斧':55,    '奇古獸':55 },
    '女妖精':     { '單手劍':75.79, '單手鈍器':51.43, '弓':60,    '十字弓':60,    '單手矛':53.4, '雙手矛':52,   '魔杖':69, '匕首':79, '雙手劍':52,    '雙手鈍器':52,    '雙刀':60,   '鋼爪':60,   '鎖鏈劍':52, '雙斧':52,    '奇古獸':52 },
    '男法師':     { '單手劍':51.43, '單手鈍器':55.38, '弓':25.26, '十字弓':25.26, '單手矛':50,   '雙手矛':48.5, '魔杖':72, '匕首':54, '雙手劍':50,    '雙手鈍器':50,    '雙刀':50,   '鋼爪':50,   '鎖鏈劍':50, '雙斧':50,    '奇古獸':50 },
    '女法師':     { '單手劍':51.43, '單手鈍器':53.33, '弓':25.71, '十字弓':25.71, '單手矛':51.6, '雙手矛':50,   '魔杖':74, '匕首':54, '雙手劍':50,    '雙手鈍器':50,    '雙刀':50,   '鋼爪':50,   '鎖鏈劍':50, '雙斧':50,    '奇古獸':50 },
    '男黑暗妖精': { '單手劍':75.78, '單手鈍器':65.45, '弓':51.42, '十字弓':68,    '單手矛':55,   '雙手矛':53,   '魔杖':60, '匕首':79, '雙手劍':55,    '雙手鈍器':55,    '雙刀':84.7, '鋼爪':75.78,'鎖鏈劍':55, '雙斧':55,    '奇古獸':55 },
    '女黑暗妖精': { '單手劍':72,    '單手鈍器':62.61, '弓':53.33, '十字弓':72,    '單手矛':55,   '雙手矛':53,   '魔杖':60, '匕首':76, '雙手劍':52,    '雙手鈍器':52,    '雙刀':90,   '鋼爪':72,   '鎖鏈劍':52, '雙斧':52,    '奇古獸':52 },
    '男龍騎士':   { '單手劍':68,    '單手鈍器':65.45, '弓':30,    '十字弓':30,    '單手矛':68,   '雙手矛':66,   '魔杖':60, '匕首':60, '雙手劍':65,    '雙手鈍器':65,    '雙刀':60,   '鋼爪':60,   '鎖鏈劍':68, '雙斧':60,    '奇古獸':60 },
    '女龍騎士':   { '單手劍':70,    '單手鈍器':62.61, '弓':31.3,  '十字弓':31.3,  '單手矛':66,   '雙手矛':64,   '魔杖':60, '匕首':62, '雙手劍':62.61, '雙手鈍器':62.61, '雙刀':60,   '鋼爪':60,   '鎖鏈劍':70, '雙斧':60,    '奇古獸':60 },
    '男戰士':     { '單手劍':60,    '單手鈍器':65.45, '弓':30,    '十字弓':30,    '單手矛':66,   '雙手矛':64,   '魔杖':60, '匕首':60, '雙手劍':66,    '雙手鈍器':66,    '雙刀':60,   '鋼爪':60,   '鎖鏈劍':60, '雙斧':72,    '奇古獸':60 },
    '女戰士':     { '單手劍':57.6,  '單手鈍器':65.45, '弓':31.3,  '十字弓':31.3,  '單手矛':68,   '雙手矛':66,   '魔杖':60, '匕首':60, '雙手劍':62.61, '雙手鈍器':62.61, '雙刀':60,   '鋼爪':60,   '鎖鏈劍':60, '雙斧':75.79, '奇古獸':60 },
    '男幻術士':   { '單手劍':55,    '單手鈍器':65.45, '弓':57.6,  '十字弓':57.6,  '單手矛':50,   '雙手矛':48,   '魔杖':72, '匕首':55, '雙手劍':50,    '雙手鈍器':50,    '雙刀':50,   '鋼爪':50,   '鎖鏈劍':50, '雙斧':50,    '奇古獸':72 },
    '女幻術士':   { '單手劍':55,    '單手鈍器':62.61, '弓':60,    '十字弓':60,    '單手矛':50,   '雙手矛':48,   '魔杖':70, '匕首':55, '雙手劍':48,    '雙手鈍器':48,    '雙刀':50,   '鋼爪':50,   '鎖鏈劍':50, '雙斧':50,    '奇古獸':74 }
};
