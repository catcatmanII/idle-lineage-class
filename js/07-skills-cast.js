...
function castSkillInner(skId) {
    ...
    if(sk.type === 'atk' && (player.cds.atkSk <= 0 || _echoFree || _royalFreeCast)) {   // 🏅 迴響精通／👑 魔法精通：免費施放不受攻擊冷卻限制
        ...
        if(sk.dmgType === 'physical') {
            ...
            for(let h = 0; h < hits; h++) {
                ...
                t.curHp -= res.dmg;
                if (res.dmg > 0 && t._reflectPhys && t.curHp > 0) { let _rfd = Math.max(1, Math.floor(res.dmg * t._reflectPhys / 100)); player.hp -= _rfd; logCombat(`<span class="text-red-400">【反射物理】</span>${t.n} 反射了 ${_rfd} 點物理傷害。`, 'enemy'); if (player.hp <= 0) killPlayer(); }
    ...
    let magicCritMult = isCrit ? (1 + player.d.magicCritDmg / 100) : 1.0;
                ...
                if(dmgArray.length > 0) {
                    ...
                    t.curHp -= totalDmg;
                    if (totalDmg > 0 && t._reflectEle && t.curHp > 0) { let _rfd = Math.max(1, Math.floor(totalDmg * t._reflectEle / 100)); player.hp -= _rfd; logCombat(`<span class="text-purple-400">【反射元素】</span>${t.n} 反射了 ${_rfd} 點元素傷害。`, 'enemy'); if (player.hp <= 0) killPlayer(); }
