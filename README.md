...
改版說明：地圖詞綴、防作弊、中毒改版（疊層）、流血改版（160%/210%）、傭兵優化、QoL 浮動數字、加速齒輪、1.8 風格 UI。
...
每次攻擊 50% 機率附加中毒（**劇毒精通**：100% 觸發）。每層中毒每秒造成該次攻擊 **10%** 傷害（**劇毒精通：20%**），持續 5 秒，**可無限疊層**（各層獨立計時、各自到期消失）。
...
## 🩸 流血傷害（`34-bleed-overhaul.js`）

匕首/矛命中時以 **力量/60** 機率觸發流血（**出血精通**：雙刀也比照觸發）。
...
| `33-poison-overhaul.js` | 中毒傷害改版：無限疊層、10%/20%覆蓋 |
| `34-bleed-overhaul.js` | 流血傷害改版：1層/5秒/160%（精通210%） |
